...
当前 P0、P1 和 P2 的本轮公开化修缮已完成，进入持续维护审计。这里的“完成”指本文件列出的本轮落地项已经有代码、测试和 CI 验证；不代表项目不再需要长期迭代。
...
- 已继续拆分 `ui/history.js`，迁出持仓表、限价挂单、公司行动和交易决策历史的 UI 渲染，并补充恶意文本转义测试。
...
- 已将案例库体检和长期画像改为默认折叠的高级分析区，减少新手首屏信息量。
- 保持现有本地运行方式和业务逻辑不变。

## 剩余长期观察项

这些不是本轮必须阻塞发布的问题，但后续继续维护时应该盯住：

- `app.js` 仍然偏大。本轮已经完成计划中的四个 UI 拆分项，并额外拆出历史记录 UI；事件处理、页面状态编排和部分渲染函数仍可继续低风险迁出。
- 案例库数量继续增加后，要优先扩充真实历史案例和来源核对流程，而不是先做复杂云架构。
- `localStorage` 对个人 MVP 足够，但如果训练记录明显变多，v1.1 可以评估 IndexedDB 或本地文件备份提醒。
- 暂时仍不建议加入真实券商下单、实时行情、登录、多用户同步、微服务、期权、融资、做空或高频策略。
