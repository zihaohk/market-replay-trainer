...
- 已补复盘后导出完整备份提醒，降低浏览器本地记录丢失风险。
- 已拆分 `ui/trade-form.js`，先迁出交易表单、订单预览、仓位计算器提示和下单教练的 UI 渲染。
- 已拆分 `ui/import-export.js`，先迁出 CSV 体检、案例包体检和案例库导入摘要的 UI 渲染。
- 已补案例包导入失败后重新导入成功的 UI 恢复回归测试。
- 已补旧版 `localStorage` / 旧备份 profile 迁移保护，避免缺少新字段时清空长期画像。
- 已把“今日建议训练”提升到首屏顶部启动条，让新手打开后先看到推荐案例和开始按钮。
- 已把复盘页“三个最该先改的问题”提升到资料包和详细指标之前，降低新手复盘阅读负担。
- 保持现有本地运行方式和业务逻辑不变。
